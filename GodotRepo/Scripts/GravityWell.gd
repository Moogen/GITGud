...
@onready var horizon_area       : Area2D            = $EventHorizon
...
@onready var horizon_shape      : CollisionShape2D  = $EventHorizon/CollisionShape2D
...
const horizon_gravity   : float = 3000*10
const blackhole_size    : float = 83*5
const horizon_size      : float = 83
...
func _ready():
    ...
    horizon_area.gravity = horizon_gravity * (click_timer_scale * click_time)
    grav_shape.shape.radius = blackhole_size * click_timer_scale * click_time
    horizon_shape.shape.radius = horizon_size * click_timer_scale * click_time
...
func remove_gravity():
    ...
    horizon_area.gravity_space_override = Area2D.SPACE_OVERRIDE_DISABLED
    horizon_area.linear_damp_space_override = Area2D.SPACE_OVERRIDE_DISABLED
